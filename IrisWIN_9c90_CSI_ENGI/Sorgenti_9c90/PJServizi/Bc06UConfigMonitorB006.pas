...
begin
  ...
  DBGrid1.Columns[7].PickList.Assign(Bc06FMonitorB006DtM.ListaEmailAuthType);
end;
...
begin
  ...
  begin
    OldPwd:=Bc06FMonitorB006DtM.selI190.FieldByName('D_EMAIL_SMTP_PASSWORD').AsString;
    NuovaPwd:=InputBox('Cambia password','Nuova password:',OldPwd);
    if NuovaPwd <> OldPwd then
      Bc06FMonitorB006DtM.selI190.FieldByName('EMAIL_SMTP_PASSWORD').AsString:=R180Cripta(NuovaPwd,SALT_CRIPTA);
  end;
end;
...
procedure TBc06FConfigMonitorB006.dgrdImpostazioniEditButtonClick(
  Sender: TObject);
var
...
begin
    ...
    begin
      if False and (not SolaLettura) and (Bc06FMonitorB006DtM.selI191.State in [dsEdit,dsInsert]) then
      begin
        OldPwd:=Bc06FMonitorB006DtM.selI191.FieldByName('D_CONNESSIONE_PWD_DECRYPT').AsString;
        NuovaPwd:=InputBox('Cambia password','Nuova password:',OldPwd);
        if (NuovaPwd <> '') and (NuovaPwd <> OldPwd) then
        begin
          NuovaPwd:=R180Cripta(NuovaPwd,SALT_CRIPTA);
          Bc06FMonitorB006DtM.selI191.FieldByName('CONNESSIONE_PWD').AsString:=NuovaPwd;
        end;
      end;
    end
    ...
    begin
      ...
      try
        C012StringList.Text:=Bc06FMonitorB006DtM.selI191.FieldByName(CampoSelezionato).AsString;
        if (not SolaLettura) and (Bc06FMonitorB006DtM.selI191.State in [dsEdit,dsInsert]) then
        begin
          OpenC012VisualizzaTesto('Modifica query','',C012StringList,'',[mbOK,mbCancel]);
          if Trim(C012StringList.Text) <> Trim(Bc06FMonitorB006DtM.selI191.FieldByName(CampoSelezionato).AsString) then
            Bc06FMonitorB006DtM.selI191.FieldByName(CampoSelezionato).AsString:=Trim(C012StringList.Text);
        end
        else
        begin
...
begin
  if Bc06FMonitorB006DtM.selI190.RecordCount < 2 then
    Abort;
  inherited;
  Modificato:=True;
end;
