...
begin
  if not FileExists('TMVW_medp.exe') then
  begin
    if MessageDlg('Il file TMVW_medp.exe necessario per la teleassistenza non è disponibile.' + CRLF + 'Eseguire il download automatico?',mtConfirmation,[mbYes,mbNo],0) = mrNo then
      exit;
    IdFtp1.Connect;
    IdFtp1.ChangeDir('download');
    IdFtp1.Get('TMVW_medp.exe','TMVW_medp.exe',True);
    IdFtp1.Disconnect;
  end;
  ShellExecute(self.WindowHandle,'open',PChar('TMVW_medp.exe'),nil,nil, SW_SHOWNORMAL)
end;
...
procedure TA078FRichiestaAssistenza.actDownloadTeamViewerExecute(
  Sender: TObject);
begin
  if FileExists('TMVW_medp.exe') then
  begin
    if MessageDlg('Il file TMVW_medp.exe è già disponibile.' + CRLF + 'Si vuole rifare il download?',mtConfirmation,[mbYes,mbNo],0) = mrNo then
      exit;
    ShellExecute(self.WindowHandle,'open',PChar('http://www.mondoedp.com/uploads/media/irisftp/download/TMVW_medp.exe'),nil,nil, SW_SHOWNORMAL);
  end;
end;
