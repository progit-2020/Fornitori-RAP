...
function DataOutLim(DataIn:TDateTime; Titolo:String; FLAG:Char; pDataMin:TDateTime=0; pDataMax:TDateTime=0; bSvuotaData:boolean=False):TDateTime;
...
{Restituisce la data}
begin
  Result:=DataOutLim(DataIn, Titolo, FLAG, 0,0, bSvuotaData);
end;

function DataOutLim(DataIn:TDateTime; Titolo:String; FLAG:Char; pDataMin:TDateTime=0; pDataMax:TDateTime=0; bSvuotaData:boolean=False):TDateTime;
{Restituisce la data - imposta le date limite inferiore e superiore}
begin
  A003FDataLavoroBis:=TA003FDataLavoroBis.Create(Application);
  with A003FDataLavoroBis do
    try
      DateTimePicker1.MinDate:=pDataMin;
      DateTimePicker1.MaxDate:=pDataMax;
      Caption:=Titolo;
      if DataIn = 0 then
