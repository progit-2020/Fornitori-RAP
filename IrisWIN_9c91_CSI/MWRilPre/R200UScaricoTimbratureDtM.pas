...
//============================================================================
    ...
    // AOSTA_REGIONE - commessa 2013/118 SVILUPPO#6.ini
    // IFunzione non è valorizzato, per cui viene commentato
    // al suo posto inserisco la chiave per successiva elaborazione
    //ITimb:=IData + IBadge + IEdbadge + IOra + IVerso + IRilev + ICausale{ + IFunzione};
    ITimb:=IData + IBadge + IEdbadge + IOra + IVerso + IRilev + ICausale + IChiave;
    // AOSTA_REGIONE - commessa 2013/118 SVILUPPO#6.fine
...
{Registrazione timbrature irregolari su I101_TIMBIRREGOLARI
 IData + IBadge + IEdbadge + IOra + IVerso + IRilev + ICausale;}
var
  A,M,G,H,N,Sec,X:Integer;
begin
  A:=StrToIntDef(Copy(S,1,4),0);
  M:=StrToIntDef(Copy(S,5,2),1);
  G:=StrToIntDef(Copy(S,7,2),1);
  H:=StrToIntDef(Copy(S,26,2),0);
  N:=StrToIntDef(Copy(S,28,2),0);
  Sec:=StrToIntDef(Copy(S,30,2),0);
  for X:=1 to Length(Trim(Copy(S,9,15))) do
  begin
    if not R180IsDigit(Trim(Copy(S,9,15)),X) then
      exit;
  end;
  with QI101 do
  begin
    ...
    ClearVariables;
    SetVariable('Badge',StrToFloatDef(Copy(S,9,15),0));
    SetVariable('EdBadge',Copy(S,24,2));
    // AOSTA_REGIONE - commessa 2013/118 SVILUPPO#6.fine
    SetVariable('Data',EncodeDate(A,M,G));
    SetVariable('Ora',EncodeDate(1900,1,1) + EncodeTime(H,N,Sec,0));
    SetVariable('Verso',Copy(S,32,1));
    SetVariable('Rilev',Copy(S,33,2));
    SetVariable('Causale',Copy(S,35,5));
    // AOSTA_REGIONE - commessa 2013/118 SVILUPPO#6.ini
    // estrae il dato chiave in alternativa al badge
    SetVariable('Chiave',Trim(Copy(S,40,20)));  // trim?? la causale non è trimmata!
    // AOSTA_REGIONE - commessa 2013/118 SVILUPPO#6.fine
