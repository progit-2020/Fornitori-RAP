...
type
  ...
  public
    ...
    procedure InsUltimeAutorizzazioni(PDaLivello:Integer; PStato,PResponsabile,PAutorizzAutomatica,PNote:String; PObbligatorie:Boolean);
    ...
    function  SetIconaAllegati(PImg: TmeIWImageFile): Boolean;
    ...
    //Proprietà
    property IterCodForm:String read GetIterCodForm;
...
begin
  ...
  if WR000DM.Responsabile and UtilizzoAvviso then
...
begin
  ...
  begin
    if WR000DM.Responsabile and FEsisteAutorizzAutomatica then
      Result:=Result + [trAutorizzAuto];
  end
  else if Iter = ITER_STRGIORNO then
  begin
...
begin
  ...
  begin
    ...
    if (not WR000DM.Responsabile) and FEsisteAutorizzIntermedia then
      Result:=Result + [trDaDefinire];
    // AOSTA_REGIONE.fine
  end
  else if Iter = ITER_STRMESE then
...
begin
  ...
  MaxLivAutorizzato:=0;
  if (PAutorizzAutomatica = '') and (Iter = ITER_STRMESE) then //Alberto 17/10/2011: se la chiamata arriva da W024 valuto l'autorizzazione automatica
...
procedure TC018FIterAutDM.InsUltimeAutorizzazioni(PDaLivello:Integer; PStato,PResponsabile,PAutorizzAutomatica,PNote:String; PObbligatorie:Boolean);
var Liv:Integer;
begin
  FMessaggioOperazione:='';
  FInibisciMailPerRichiedente:=True;
...
{$IFNDEF WEBSVC}
{$IFDEF IRISWEB}
function TC018FIterAutDM.SetIconaAllegati(PImg: TmeIWImageFile): Boolean;
...
begin
  ...
  try
    ...
    // valuta gestione allegati per richiesta
