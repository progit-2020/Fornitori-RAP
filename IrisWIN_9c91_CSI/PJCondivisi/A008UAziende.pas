...
begin
  ...
  for i:=1 to High(DatiEnte) do
    ...
    begin
      ...
      else if DatiEnte[i].Lista = 'OFFICE' then
      begin
...
begin
  actCancella.Enabled:=(DButton.State = dsBrowse) and (PageControl1.ActivePageIndex = 0);
  TCanc.Enabled:=(DButton.State  = dsBrowse)and (PageControl1.ActivePageIndex = 0);
  actInserisci.Enabled:=(DButton.State  = dsBrowse) and (PageControl1.ActivePageIndex = 0);
  TInser.Enabled:=(DButton.State  = dsBrowse) and (PageControl1.ActivePageIndex = 0);
end;

procedure TA008FAziende.TAnnullaClick(Sender: TObject);
begin
  inherited;
  with A008FOperatoriDtM1 do
  begin
    SessioneOracle.CancelUpdates([selI093,selI095,A181MW.selI094,A181MW.selI097,selI096]);
...
procedure TA008FAziende.TInserClick(Sender: TObject);
begin
  inherited;
  A008FOperatoriDtM1.selI095.ReadOnly:=False;
  A008FOperatoriDtM1.selI096.ReadOnly:=False;
end;
...
begin
  ...
  begin
    ...
    A181MW.selI094.ReadOnly:=False;
    selI095.ReadOnly:=False;
    A181MW.selI097.ReadOnly:=False;
...
procedure TA008FAziende.dgrdselI095EditButtonClick(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    begin
      A008FIterCondizValidita:=TA008FIterCondizValidita.Create(Self);
      try
        A008FIterCondizValidita.Caption:=dgrdselI095.SelectedField.DisplayLabel;
        A008FIterCondizValidita.dsrGenerale.DataSet:=A008FOperatoriDtm1.A181MW.selI097;
        A008FIterCondizValidita.ShowModal;
      finally
        FreeAndNil(A008FIterCondizValidita);
      end;
    end
    else
...
procedure TA008FAziende.dgrdselI096ColEnter(Sender: TObject);
begin
  inherited;
...
procedure TA008FAziende.dgrdselI096EditButtonClick(Sender: TObject);
begin
  inherited;
  EditaCampoMemo(dgrdselI096,dgrdselI096.SelectedField.FieldName);
end;

procedure TA008FAziende.dgrdselI093EditButtonClick(Sender: TObject);
begin
  inherited;
  with A008FOperatoriDtm1 do
  begin
    ...
    begin
      ...
      begin
        A008FIterCondizValidita:=TA008FIterCondizValidita.Create(Self);
        try
          A008FIterCondizValidita.Caption:=dgrdselI093.SelectedField.DisplayLabel;
          A008FIterCondizValidita.dsrGenerale.DataSet:=A181MW.selI094;
          A008FIterCondizValidita.ShowModal;
        finally
          FreeAndNil(A008FIterCondizValidita);
        end;
...
procedure TA008FAziende.pmnLivelliPopup(Sender: TObject);
begin
  inherited;
  actImpostaScriptDefault.Visible:=(A008FOperatoriDtm1.selI093.FieldByName('ITER').AsString = ITER_SCIOPERI) and
                                   (not SolaLettura) ;
end;

procedure TA008FAziende.actImpostaScriptDefaultExecute(Sender: TObject);
var
  OldReadOnly: Boolean;
begin
  inherited;
...
procedure TA008FAziende.actRefreshExecute(Sender: TObject);
begin
  inherited;
  With A008FOperatoriDtm1 do
  begin
