...
uses
  ...
  A000UInterfaccia, C700USelezioneAnagrafe, System.IOUtils,
  ...
  C001UScegliCampi, C180FunzioniGenerali,  C021UDocumentiManagerDM,
  SelAnagrafe, A120UTIPIRIMBORSI, A129UIndennitaKm, Variants, QueryStorico,
  L001Call, C010USelezioneDaElenco, System.UITypes, WinApi.ShellApi,
...
type
  TA100FMISSIONI = class(TR001FGestTab)
    ...
    tabAllegati: TTabSheet;
    dgrdAllegati: TDBGrid;
    ActionList2: TActionList;
    actFileVisualizza: TAction;
    actFileDownload: TAction;
    pmnAllegati: TPopupMenu;
    Apri1: TMenuItem;
    Salva1: TMenuItem;
...
begin
  ...
  dgrdAllegati.DataSource:=A100FMISSIONIdtm.A100FMissioniMW.dsrT960;
  DButton.DataSet:=A100FMISSIONIDTM.M040;
  FC021DM:=TC021FDocumentiManagerDM.Create(nil);
...
// toolbar dettaglio giornaliero
  ...
  pnlCheck.Visible:=(chkIndennitaSupMaxGG.Visible) or (dChkModifica.Visible);
  // imposta la visibilità del tab "Allegati alle richieste"
  tabAllegati.TabVisible:=A100FMISSIONIdtm.A100FMissioniMW.GestioneAllegatiRichieste;
end;
...
procedure TA100FMISSIONI.actFileDownloadExecute(Sender: TObject);
...
begin
  // estrae id allegato
  LId:=dgrdAllegati.DataSource.DataSet.FieldByName('ID').AsInteger;
  ...
  try
    try
      ...
      on E: Exception do
      begin
        R180MessageBox(Format('Errore durante il download dell''allegato: %s',[E.Message]),ESCLAMA);
        Exit;
      end;
    end;
  finally
...
procedure TA100FMISSIONI.actFileVisualizzaExecute(Sender: TObject);
