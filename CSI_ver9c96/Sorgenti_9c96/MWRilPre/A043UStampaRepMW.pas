...
type
  ...
  TA043FStampaRepMW = class(TR005FDataModuleMW)
    ...
    QCausaliAssenzaINFLUCONT: TStringField;
...
begin
  ...
  if QCausaliAssenza.Locate('Codice',Causale,[loCaseInsensitive]) then
    Result:=(QCausaliAssenza.FieldByName('DetReperib').AsString <> '0') and (QCausaliAssenza.FieldByName('InfluCont').AsString <> 'D');
end;
...
begin
  ...
  begin
    ...
    //18/05/2022: Se la causale diminuisce le ore lavorate, la annullo riportando la valenza nelle ore lavorate perchè non devono mai diminuire l'abbattimento del turno
    if (R502ProDtM1.ValStrT265[R502ProDtM1.triepgiusasse[i].tcausasse,'INFLUCONT'] = 'D') and (VarToStr(QCausaliAssenza.Lookup('Codice',R502ProDtM1.triepgiusasse[i].tcausasse,'DetReperib')) = '1') then
      inc(OreLav,R502ProDtM1.triepgiusasse[i].tminvalasse);
  end;
  if OreNonCaus then
  begin
