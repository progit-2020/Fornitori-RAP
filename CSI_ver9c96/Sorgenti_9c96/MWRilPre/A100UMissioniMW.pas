...
type
  ...
  TA100FMissioniMW = class(TR005FDataModuleMW)
    ...
    dsrT960: TDataSource;
    selCountI095Allegati: TOracleQuery;
    Q050FLAG_NON_RIMBORSABILE: TStringField;
    Q020FLAG_NON_RIMBORSABILE: TStringField;
  ...
  public
    ...
    GestioneAllegatiRichieste: Boolean;
...
begin
  ...
  Q050.FieldByName('FLAG_NON_RIMBORSABILE').LookupDataSet:=Q020;
  ...
  *)
  ...
  // visibile in base a:
  // - modulo iter autorizzativi abilitato
  // - presenza di strutture autorizzative con condizione allegati impostata e diversa da 'N'
  if Parametri.ModuloInstallato['ITER_AUTORIZZATIVI'] then
  begin
    (*Da sbloccare nella 9c.9(5)GestioneAllegatiRichieste:=False;*)
    // valuta gestione allegati in base alle condizioni sulle strutture
    selCountI095Allegati.Execute;
    GestioneAllegatiRichieste:=selCountI095Allegati.FieldAsInteger(0) > 0;
  end
  else
  begin
    // gestione allegati disabilitata
    GestioneAllegatiRichieste:=False;
  end;

  // buffer di lettura impostato come max numero di allegati per richiesta
  if GestioneAllegatiRichieste then
    selT960.ReadBuffer:=StrToIntDef(Parametri.CampiRiferimento.C90_IterMaxAllegati,5) + 1;
end;
...
begin
  ...
  if (M051.FieldByName('SOMMA').AsString = 'S') and (Q050.FieldByName('FLAG_NON_RIMBORSABILE').AsString = 'S') then
    raise Exception.Create(A000MSG_A100_ERR_FLAG_NO_RIMB_SOMMA);
...
var
  Importo: Real;
begin
  if (Q050.FieldByName('flag_non_rimborsabile').AsString = 'S') and (Q050.FieldByName('IMPORTORIMBORSOSPESE').AsFloat <> 0) then
  begin
    raise Exception.Create(A000MSG_A100_ERR_FLAG_NO_RIMB_ZERO);
  end;
  if (Q050.ReadOnly = False) then
  begin
    Q050.FieldByName('IMPORTOINDENNITASUPPLEMENTARE').AsInteger:=0;
...
procedure TA100FMissioniMW.selT960CalcFields(DataSet: TDataSet);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      selT960.SetVariable('ID_MISSIONE',FSelM040_Funzioni.FieldByName('ID_MISSIONE').AsInteger);
      selT960.Open;
    end;
    // ==============================
    // VALORIZZO DCMBLOOKUPCODTTARIFF
