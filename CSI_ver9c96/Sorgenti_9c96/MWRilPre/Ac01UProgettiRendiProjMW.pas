...
type
  ...
  TAc01FProgettiRendiProjMW = class(TR005FDataModuleMW)
    ...
    selT750Copia: TOracleDataSet;
    scrCopiaDettaglio: TOracleQuery;
...
begin
  ...
  //Visualizzazione di tutte le causali di assenza e di presenza a cui è stato attivato il parametro storicizzato RENDICONTA_PROGETTI
  R180SetVariable(selCauIncluse,'DECORRENZA',selT750.FieldByName('DECORRENZA').AsDateTime);
  R180SetVariable(selCauIncluse,'DECORRENZA_FINE',selT750.FieldByName('DECORRENZA_FINE').AsDateTime);
  selCauIncluse.Open;
  selCauIncluse.First;
  while not selCauIncluse.Eof do
  begin
    if selCauIncluse.FieldByName('RENDICONTA_PROGETTI').AsString = 'S' then
      selT750.FieldByName('CAUASSPRES_INCLUSE_CF').AsString:=selT750.FieldByName('CAUASSPRES_INCLUSE_CF').AsString + IfThen(not selT750.FieldByName('CAUASSPRES_INCLUSE_CF').IsNull,',') + selCauIncluse.FieldByName('CODICE').AsString;
    selCauIncluse.Next;
  end;
end;
