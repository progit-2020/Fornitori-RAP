    ...
    with R003FGeneratoreStampeMW.Serbatoi[i] do
      ...
      begin
        ...
        begin
          ...
          //Applicazione filtro per Progressivo in modo da leggere pochi records per volta e impedire la lettura di query gigantesche
          ODS.SQL.Add('WHERE ' + R003FGeneratoreStampeMW.TabelleCollegate[TM].Progressivo + ' = :PROGRESSIVO');
          ODS.DeclareVariable('PROGRESSIVO',otInteger);
          if Filtro <> '' then
            ODS.SQL.Add('AND (' + Filtro + ')');
          if Pos(':DAL',UpperCase(ODS.SQL.Text)) > 0 then
          begin
            ODS.DeclareVariable('DAL',otDate);
            ODS.SetVariable('DAL', R003FGeneratoreStampe.DataI);
          ...
          //ODS.SQL.Text:=StringReplace(ODS.SQL.Text,':UTENTE','''' + ParametriOperatore + '''',[rfReplaceAll,rfIgnorecase]);
          ODS.Session:=SessioneOracle;
          ODS.ReadBuffer:=2000;
          ODS.Tag:=TM;
  ...
  begin
    ...
    begin
      ...
      for i:=0 to High(R003FGeneratoreStampeMW.Serbatoi) do
        ...
        begin
          R180SetVariable(R003FGeneratoreStampeMW.Serbatoi[i].ODS,'PROGRESSIVO',DipendenteCorrente.Progressivo);
          R003FGeneratoreStampeMW.Serbatoi[i].ODS.Open;
