...
uses
  ...
  Mask, {DBLookup,} DB, Buttons, A000UCostanti, A000USessione,A000UInterfaccia, Oracle, OracleData,
  ...
  ToolbarFiglio, System.UITypes, StrUtils
...
begin
  if (DButton.DataSet = nil) or InterfacciaR004.GestioneStoricizzata then
    dedtDecorrenza.DataField:='DECORRENZA'
  else
    dedtDecorrenza.DataField:='';
...
begin
  ...
  begin
    ...
    if (DButton.State = dsInsert) and (not InterfacciaR004.StoricizzazioneInCorso) then
      SalvaValoriOriginali; //Utile in AllineaDecorrenzaFine solo per P430
  end;
...
begin
  ...
  if Assigned(DButton.DataSet.FindField('DECORRENZA')) then
