...
type
  ...
  TB007FManipolazioneDati = class(TForm)
    ...
    procedure cmbStatoDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
    procedure cmbStatoChange(Sender: TObject);
    procedure cmbCausaliChange(Sender: TObject);
  private
...
uses B007UManipolazioneDatiDtM1, B022UUtilityGestDocumentaleMW;
...
begin
  ...
  If Assigned(B007FManipolazioneDatiDtM1) then
...
// inizializzazione TabSheet5
  ...
  for i:=0 to PageControl1.PageCount-1 do
    PageControl1.Pages[i].TabVisible:=PageControl1.Pages[i].TabVisible and A000FiltroDizionario('MANIPOLAZIONE DATI', PageControl1.Pages[i].Caption);
end;

procedure TB007FManipolazioneDati.cmbStatoChange(Sender: TObject);
begin
  with B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW do
  begin
    if cmbStato.ItemIndex <> 0 then
      R180SetVariable(selT960,'STATO', 'and T850.STATO = ''' + cmbStato.Text + '''')
    else
...
procedure TB007FManipolazioneDati.cmbStatoDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
begin
  case index of
  0: (Control as TComboBox).Canvas.TextRect(Rect,Rect.Left + 2,Rect.Top,'Tutte');
  1: (Control as TComboBox).Canvas.TextRect(Rect,Rect.Left + 2,Rect.Top,'Sì');
  2: (Control as TComboBox).Canvas.TextRect(Rect,Rect.Left + 2,Rect.Top,'No');
  end;
end;
...
procedure TB007FManipolazioneDati.CaricaCmbCausali;
var i:Integer;
    Stato:String;
begin
  Stato:=cmbStato.Text;
  cmbCausali.Items.Clear;
  cmbCausali.Enabled:=cmbCodiceIter.Text <> '';
  if not R180In(cmbCodiceIter.Text,['T050','M140']) then
    exit;
  if cmbCodiceIter.Text <> '' then
  begin
    B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.FiltraCausali(chkFiltroAnagrafe.Checked, IfThen(AllaData <= DataMaxIter, '''DB''', ''), DallaData, AllaData, cmbCodiceIter.Text, Stato);
    B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.selCausali.First;
    cmbCausali.Items.Add('Tutte');
    while not B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.selCausali.Eof do
    begin
      cmbCausali.Items.Add(B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.selCausali.FieldByName('CAUSALE').AsString);
      B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.selCausali.Next;
    end;
    cmbCausali.ItemIndex:=0;
  end;
end;

procedure TB007FManipolazioneDati.CaricaCmbIter;
var i:Integer;
begin
  cmbCodiceIter.Items.Clear;
  cmbCodiceIter.Items.Add('');
  for i:=0 to 1 (*High(A000IterAutorizzativi)*) do //solo missioni e giustificativi
...
procedure TB007FManipolazioneDati.cmbCausaliChange(Sender: TObject);
begin
  with B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW do
  begin
    if cmbCausali.ItemIndex <> 0 then
      R180SetVariable(selT960,'INFO_RICHIESTA', 'and decode(T850.ITER,''T050'',T050.CAUSALE,''M140'',M140.PROTOCOLLO) = ''' + cmbCausali.Text + '''')
    else
...
procedure TB007FManipolazioneDati.cmbCodiceIterChange(Sender: TObject);
begin
  ImpostaFiltroAllegati;
  case cmbCodiceIter.ItemIndex of
  0: lblInfo.Caption:='Info richiesta';
  1: lblInfo.Caption:='Protocollo';
  2: lblInfo.Caption:='Causale';
  end;
end;

procedure TB007FManipolazioneDati.cmbCodiceIterDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
begin
  if Index = 0 then
...
begin
  if AllaData < DallaData then
    R180MessageBox('Periodo non valido', ESCLAMA);
  ...
  B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.FiltraT960(chkFiltroAnagrafe.Checked, DallaData, Min(DataMaxIter, AllaData), IfThen(cmbCodiceIter.ItemIndex > 0, cmbCodiceIter.Text), IfThen(cmbStato.ItemIndex > 0, cmbStato.Text), IfThen(cmbCausali.ItemIndex > 0, cmbCausali.Text));
...
function DimDownload: integer;
begin
  dbgT960.DataSource.DataSet.First;
  Result:=0;
  while not dbgT960.DataSource.DataSet.eof do
  begin
    Result:=Result + dbgT960.DataSource.DataSet.FieldByName('DIMENSIONE').AsInteger;
...
var
  Scelta: Integer;
begin
  if not B007FManipolazioneDatiDtM1.B022FUtilityGestDocumentaleMW.C021DM.TestWebService then
  begin
  ...
  begin
    Scelta:=MessageDlg(R180GetFileSizeStr(DimDownload) + ' (' + IntToStr(dbgT960.DataSource.DataSet.RecordCount) + ' file) verranno spostati nell''area di storage "' +
                        edtNuovoPath.Text + '" (File system)' + CRLF +
                        'Procedere?', mtWarning, mbOKCancel,0);
    if not (Scelta = mrOk) then
      Exit;
    if B007FManipolazioneDatiDtM1.B022FUtilityGestDocumentaleMW.C021DM.TestAccesso(edtNuovoPath.Text) then
      B007FManipolazioneDatiDtM1.B022FUtilityGestDocumentaleMW.ElaborazioneRecords(0, edtNuovoPath.Text)
    else
    begin
  ...
  begin
    ...
    begin
      ...
      if not (Scelta = mrOk) then
        Exit;
      if B007FManipolazioneDatiDtM1.B022FUtilityGestDocumentaleMW.C021DM.TestAccesso(Parametri.CampiRiferimento.C90_PathAllegati) then
        B007FManipolazioneDatiDtM1.B022FUtilityGestDocumentaleMW.ElaborazioneRecords(0, Parametri.CampiRiferimento.C90_PathAllegati)
      else
      begin
        ShowMessage('Non è possibile spostare file nella nuova area di storage');
        Exit;
