...
uses
  ...
  System.Rtti
  (* MAN/07 SVILUPPO#84*),IW.Parser.Files, HTTPApp;
...
(* MAN/07 SVILUPPO#84 *)
  ...
  begin
    ...
    else if Request.Files.Count > 0 then      //Files: funziona solo in modalità stand-alone (testato su tutti i browser desktop tranne IE)
    try
    ...
    else                                      //per IE si legge direttamente la sessione da Request.PathInfo, che andrebbe bene anche negli altri casi sopra, ma l'info non è molto strutturata e non varrebbe per dati diversi dall'id di sessione.
    begin
    ...
    if S <> '' then
    begin
      //lstIdSessioniAttive per controllare se l'AfterRender è scattato prima della fine del BeforeDispatch
      ...
      begin
        try
          if lstIdSessioniAttive.IndexOf(S) < 0 then //se > 0 è scattato l'AfterRender (in anticipo) quindi il TimeOut non deve essere ridotto
          begin
...
begin
  ...
  (*
   ID di sessione nell'url o no:
   * Se impostato a true consente di aprire più sessioni anche con utenze diverse dallo stesso browser
   * (Vedi http://docs.atozed.com/docs.dll/Classes/TIWServerControllerBase.html default=True)
   * ma il codice identificativo della sessione sarà messo nelle url delle richieste, se impostato a false
   * sarà consentita una sola connessione per browser ma l'identificativo non verrà visualizzato nelle url
   Si lascia il comportamento di default (True) in attesa di ulteriori verifiche.
   *)
...
uses Classes, SysUtils, SyncObjs, Oracle, OracleData, Data.DB, A000UCostanti, A000USessione, QueryPK, RegistrazioneLog, Variants;
