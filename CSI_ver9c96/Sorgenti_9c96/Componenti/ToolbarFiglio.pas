...
uses
  ...
  Variants, ActnList, ImgList, ToolWin, System.Actions
  {$IFNDEF VER250}{<> XE4}
  , System.ImageList
  {$ENDIF}
  ;
...
begin
  ...
  //(Bruno: 17/07/2015) Se è un OracleDataSet controllo se ha il cachedUpDate disattivo
  //se disattivo asssegno DS.OnStateChange:=Self.DButtonStateChange
  if (DS.DataSet is TOracleDataSet) and not (DS.DataSet as TOracleDataSet).CachedUpdates then
    DS.OnStateChange:=Self.DButtonStateChange;
  if DS.DataSet is TOracleDataSet then
    TFReadOnly:=(DS.DataSet as TOracleDataSet).ReadOnly;
end;
...
begin
  ...
  try
    ...
    for i:=0 to TFDBGrid.Columns.Count - 1 do
      if TFDBGrid.Columns[i].Visible then
      begin
        TFDBGrid.SelectedIndex:=i;
        Break;
      end;
  except
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      TOracleDataSet(DButton.DataSet).Session.ApplyUpdates([TOracleDataSet(DButton.DataSet)],True);
