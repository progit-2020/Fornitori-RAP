...
begin
  ...
  if (Var_Gestione = 1) then
  begin
    if (Q265.FieldByName('CUMULOGLOBALE').AsString = 'C') and selConiuge.SearchRecord('TIPO','GENITORI',[srFromBeginning]) and (selConiuge.FieldByName('NUM').AsInteger = 0) then
    begin
      Errore:=A000TraduzioneStringhe(A000MSG_A004_MSG_NO_CONIUGE);
    end
    else if (Q265.FieldByName('CUMULOGLOBALE').AsString = 'F') and selConiuge.SearchRecord('TIPO','FAMILIARI',[srFromBeginning]) and (selConiuge.FieldByName('NUM').AsInteger = 0) then
    begin
      Errore:=A000TraduzioneStringhe(A000MSG_A004_MSG_NO_FAMILIARE);
    end
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      // gestione invio mail
      if Parametri.CampiRiferimento.C90_EMailW010Uff = 'I' then
      begin
        //mail configurata sull'iter per il richiedente
        C018DM.MailPerRichiedente(C018DM.LivMaxAutNeg);
      end
      else if Parametri.CampiRiferimento.C90_EMailW010Uff = 'S' then
      begin
        //mail generica
...
begin
  ...
  begin
    ...
    if Var_Gestione = 1 then
      Accept:=Accept and R180In(DataSet.FieldByName('CUMULOGLOBALE').AsString,['C','F']);
  end
