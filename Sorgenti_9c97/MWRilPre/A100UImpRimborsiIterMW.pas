...
begin
  //selM170.Close;
  R180SetVariable(selM170,'ID',FSelM150_Funzioni.FieldByName('ID').AsInteger);
  selM170.Open;

  //selM175.Close;
  R180SetVariable(selM175,'ID',FSelM150_Funzioni.FieldByName('ID').AsInteger);
  selM175.Open;

  // CUNEO_ASLCN1 - chiamata 88143.ini
  //selM143.Close;
  R180SetVariable(selM143,'ID',FSelM150_Funzioni.FieldByName('ID').AsInteger);
  selM143.Open;
  // CUNEO_ASLCN1 - chiamata 88143.fine
end;
...
begin
  ...
  begin
    try
      with selDatoSede do
      begin
        R180SetVariable(selDatoSede,'DATOSEDE',Parametri.CampiRiferimento.C8_Sede);
        R180SetVariable(selDatoSede,'PROGRESSIVO',Progressivo);
        R180SetVariable(selDatoSede,'DATARIF',Parametri.DataLavoro);
        Open;
        if RecordCount > 0 then
        begin
          CodComuneSede:=Fields[0].AsString;
          CapComuneSede:=Fields[1].AsString;
          DescComuneSede:=Fields[2].AsString;
        end;
      end;
    except
    end;
