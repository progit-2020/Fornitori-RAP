...
{Implementeazione Quick Sort per ordinamento timbrature}
  ...
  begin
    Lo:=iLo;
    Hi:=iHi;
    Mid:=R180VarToDateTime(FTimbrature[Day,(Lo + Hi) div 2].Ora);
    repeat
      while R180VarToDateTime(FTimbrature[Day,Lo].Ora) < Mid do Inc(Lo);
      while R180VarToDateTime(FTimbrature[Day,Hi].Ora) > Mid do Dec(Hi);
      if Lo <= Hi then
      begin
...
begin
  ...
  begin
    if (VarIsNull(Ora)) or
       (R180VarToDateTime(Ora) <> QTimbrature.FieldByName('Ora').AsDateTime) then Result:=True;
