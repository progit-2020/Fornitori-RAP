...
const
  ...
  XP_FLEX_ACCORCIA_DETPMT   = '<FLEX_ACCORCIA_DETPMT>';
  XP_FLEX_ACCORCIA_DETPMA   = '<FLEX_ACCORCIA_DETPMA>';
  XP_COTO_ORECAUS_DETPM     = '<COTO_ORECAUS_DETPM>';
  XP_CRV_ALLARGA_PMT        = '<CRV_ALLARGA_PMT>';
  XP_REC_PSICOFISICO        = '<REC_PSICOFISICO>';
  XP_Z1004_DOPO_COMPDEBITO  = '<Z1004_DOPO_COMPDEBITO>';
  ...
  XP_GIUSTIF_TURNO_NOTTURNO = '<GIUSTIF_TURNO_NOTTURNO>';
  ...
  XP_TC_z352_OLD            = '<TC_z352_OLD>';
  XP_OLD_ASS_OLTREDEBITO    = '<OLD_ASS_OLTREDEBITO>';
  XP_GIUST_PAUMENDET        = '<GIUST_PAUMENDET>';
  XP_INTERSEZPMT_ESTERNA    = '<INTERSEZPMT_ESTERNA>';
  XP_v77_z560               = '<v77_z560>';
  XP_TC_TURNOPIANIFICATO    = '<TC_TURNOPIANIFICATO>';
  XP_TORINO_CSI_PRV         = '<TORINO_CSI_PRV>';
...
type
  ...
  private
    ...
    procedure z1004_ArrotondaOreCausalizzate(Dopo_CompensoDebitoDaCausEscluse:Boolean = False);
    ...
    procedure z1013_ArrotondaOreCausalizzateSGM(Dopo_CompensoDebitoDaCausEscluse:Boolean = False);
...
begin
  ...
  *)
  ...
  //Dati storicizzati
    ...
    Result:=lstT235.ValoreCorrente[Dato]
  else if UpperCase(Dato) = 'ARROT_RIEPGG_FINECONT' then
    Result:=lstT235.ValoreCorrente[Dato];
end;
...
begin
  ...
  R180SetVariable(selRendiProj,'DATA',datacon);
  selRendiProj.Open;
  //Includo le causali di assenza specificate tranne quelle a gg intera
  for i:=1 to n_riepasse do
    if selRendiProj.SearchRecord('CODICE',triepgiusasse[i].tcausasse,[srFromBeginning]) then
      //if triepgiusasse[i].ttipofruiz <> 'I' then
        inc(Result,triepgiusasse[i].tminvalasse);
  //Includo le causali di presenza specificate
  for i:=1 to n_rieppres do
    if selRendiProj.SearchRecord('CODICE',triepgiuspres[i].tcauspres,[srFromBeginning]) then
      inc(Result,RiepPresTotale[i]);
end;
...
{Azzeramenti iniziali}
  ...
  begin
    ...
    begin
      ...
      if (chiamante <> 'Assenze') and (TipoOrario = 'C') and (i = 3) then
        z200_oreelast;
    ...
    begin
      ...
      //Prima gestione dell'arrotondamento della causale, prima della compensazione del debito
      ...
      //Seconda gestione dell'arrotondamento della causale, dopo la compensazione del debito
      if (not XParam['<SGM_CAUSARR_FASCE>']) or (datacon < EncodeDate(2013,11,1)) then
        //Torino_Comune: arrotondamento riepiloghi causali ai 30 minuti con mantenimento del resto nelle ore rese per scorrimento
        z1004_ArrotondaOreCausalizzate(True)
      else
        //S.Giuliano Milanese: solo dal 01/11/2013 arrotondamento in fasce con riporto del resto sulla fascia più bassa
        z1013_ArrotondaOreCausalizzateSGM(True);
...
begin
  ...
  begin
    ...
    begin
      ...
      //Gestione libera professione (prima chiamata - nuovo comportamento del 05/03/2021)
      if (cdsT020.FindField('POSTICIPA_CAUS_TIMB_INTERSEC') <> nil) and (cdsT020.FieldByName('POSTICIPA_CAUS_TIMB_INTERSEC').AsString <> 'S') then
      ...
      //Gestione libera professione (seconda chiamata - comportamento standard)
      if (cdsT020.FindField('POSTICIPA_CAUS_TIMB_INTERSEC') = nil) or (cdsT020.FieldByName('POSTICIPA_CAUS_TIMB_INTERSEC').AsString = 'S') then
        z550_LiberaProfessione;
...
var i:Integer;
  ...
  begin
    ...
    begin
      ...
      if (Blocca = 0) and (cdsT020.FindField('ANOM_BLOCC_23LIV') <> nil) and R180In('2_' + tanom2riscontrate[i].ta2puntdesc.ToString,cdsT020.FieldByName('ANOM_BLOCC_23LIV').AsString.Split([','])) then
      begin
        Blocca:=25;
    ...
    begin
      ...
      if (Blocca = 0) and (cdsT020.FindField('ANOM_BLOCC_23LIV') <> nil) and R180In('3_' + tanom3riscontrate[i].ta3puntdesc.ToString,cdsT020.FieldByName('ANOM_BLOCC_23LIV').AsString.Split([','])) then
      begin
        Blocca:=26;
  ...
  begin
    ...
    begin
      S:='';
      if tanom3riscontrate[i].ta3puntdesc in [4,6,11] then
        S:=tdescanom3[tanom3riscontrate[i].ta3puntdesc].D + ' ' + R180MinutiOre(tanom3riscontrate[i].ta3timb) + ' ore: ' + tanom3riscontrate[i].ta3desc
...
{Scarto timbrature con causale presenza
 su U e successiva E di tipo conteggio A (se abilitate)}
var i:Integer;
  function CheckCausaleAccoppiata(pPrecSucc:Integer; pCaus:String):Boolean;
  begin
    Result:=False;
    if XParam['<v9c93_z072>'] then
      exit;
    ...
    try
      ...
      if pPrecSucc = -1 then
        SQL.Text:='select max(CAUS_E) from VT100_COPPIE_EU where PROGRESSIVO = :PROGRESSIVO and DATA_U = :DATA and oreminuti(ORA_U) = :ORA and CAUS_U = :CAUSALE'
      else
        SQL.Text:='select max(CAUS_U) from VT100_COPPIE_EU where PROGRESSIVO = :PROGRESSIVO and DATA_E = :DATA and oreminuti(ORA_E) = :ORA and CAUS_E = :CAUSALE';
      DeclareAndSet('PROGRESSIVO',otInteger,ProgrCon);
      DeclareAndSet('DATA',otDate,datacon + pPrecSucc);
      DeclareAndSet('ORA',otString,IfThen(pPrecSucc = -1,minuti_pre,minuti_suc));
      DeclareAndSet('CAUSALE',otString,IfThen(pPrecSucc = -1,caus_pre,caus_suc));
      Execute;
      Result:=FieldAsString(0) = pCaus;
    finally
      Free;
    end;
  end;
begin
  ...
  begin
    ...
    ttimbraturedip[1].tcausale_e.tcaus:='';
    primat_u:='si';
    //Gestione riporto della causale sull'uscita anche sull'entrata
    if (ttimbraturedip[1].tcausale_u.tcaus <> '')
    and (ttimbraturedip[1].tcausale_u.tcausabi = 'si')
    and (ttimbraturedip[1].tcausale_u.tcaustip = 'B')
    and (not R180In(ttimbraturedip[1].tcausale_u.tcauscon,['A','E']))
    then
    begin
      if CheckCausaleAccoppiata(-1,ttimbraturedip[1].tcausale_u.tcaus) then
        ttimbraturedip[1].tcausale_e:=ttimbraturedip[1].tcausale_u;
    end;
  end;
  ...
  begin
    ...
    ttimbraturedip[n_timbrdip].tcausale_u.tcaus:='';
    ultimt_e:='si';
    //Gestione riporto della causale sull'entrata anche sull'uscita
    if (ttimbraturedip[n_timbrdip].tcausale_e.tcaus <> '')
    and (ttimbraturedip[n_timbrdip].tcausale_e.tcausabi = 'si')
    and (ttimbraturedip[n_timbrdip].tcausale_e.tcaustip = 'B')
    and (not R180In(ttimbraturedip[n_timbrdip].tcausale_e.tcauscon,['A','E']))
    then
    begin
      if CheckCausaleAccoppiata(1,ttimbraturedip[n_timbrdip].tcausale_e.tcaus) then
        ttimbraturedip[n_timbrdip].tcausale_u:=ttimbraturedip[n_timbrdip].tcausale_e;
    end;
  end;
...
begin
  ...
  if XParam[XP_FLEX_ACCORCIA_DETPMT] then
    PauMenMinUtilizzata:=PauMenMinUtilizzata - min(FlessibilitaRit,PauMenMinUtilizzata);
...
var Abilitato:Boolean;
  ...
  begin
    Result:=Abilitato;
    if cdsT020.FindField('SEPARA_TIMB_CONTEGGI') = nil then
    begin
    ...
    begin
      if cdsT020.FieldByName('SEPARA_TIMB_CONTEGGI').AsString = 'S' then (*not XParam[XP_PROLUNGA_TIMB_SPEZZATE] then*)
        exit;
    end;
    ...
    begin
      ...
      begin
        ...
        if not cdsT020.FieldByName('PausaMensa_Automatica').IsNull then
          ...
          PauMenMinUtilizzata:=ValNumT020['mmMinimi'];
        if XParam[XP_FLEX_ACCORCIA_DETPMA] then
          PauMenMinUtilizzata:=PauMenMinUtilizzata - min(FlessibilitaRit,PauMenMinUtilizzata);
...
begin
  ...
  if gglav = 'si' then
    if (n_timbrdip > 0) or ((n_timbrdip = 0) and (n_giusgga = 0) and (n_giusmga < 2)) then
    begin
  ...
  begin
    ...
    begin
      ...
      if aum862 = 'si' then
        ...
        dec(minlavorar,min(U - E, minlavorar));
...
var App,App2,FruizMin,FruizArr,giustDalle,giustAlle,minvalcompass:Integer;
  ...
  begin
    ...
    begin
      //Gerstisco il caso particolare di giustificativo che lascia inalterate o diminusce le ore, interseca le timrbature, e le timbrature vengono comunque conteggiate
      if R180CarattereDef(causinf) in ['B','D'] then
      begin
        if ValStrT265[Causale,'INTERSEZIONE_TIMBRATURE'] = 'E' then
          for i:=1 to n_timbrdip do
          begin
            if (ttimbraturedip[i].tag <> 'TG=ASSENZA') and (dalle < ttimbraturedip[i].tminutid_u) and (alle > ttimbraturedip[i].tminutid_e) then
            begin
              ...
              if dalle > alle then
                alle:=dalle;
              if (dalle < ttimbraturedip[i].tminutid_e) and (alle > ttimbraturedip[i].tminutid_u) then
                alle:=alle - (ttimbraturedip[i].tminutid_u - ttimbraturedip[i].tminutid_e);
              Break;
            end;
          end;
      end;
...
var Locl04_alle,Locl04_dalle,i:Integer;
  ...
  begin
    ...
    GiustificativiR600[i].MatricolaFamiliare:='';
  end;
begin
...
var i,k,mm,app,tot,n_fo,e_fo,u_fo,e_tn,u_tn,TotFasciaObb:Integer;
  ...
  begin
    ...
    begin
      ...
      //In fascia facoltativa il periodo della PMT non è considerato
      if (not Esiste) and (TipoFascia = 'F') and R180Between(mmFascia,InizioMensa + 1,FineMensa - 1) then
        Esiste:=True;
      F:=TipoFascia;
      if Esiste then
        F:=F + 'P'
...
begin
  ...
  begin
    ...
    begin
      e_tn:=ValNumT021['ENTRATA_OBB',TF_PUNTI_NOMINALI,1];
      u_tn:=ValNumT021['USCITA_OBB',TF_PUNTI_NOMINALI,1];
    end
    else
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        inc(n_anom3);
        SetLength(tanom3riscontrate,n_anom3 + 1);
        tanom3riscontrate[n_anom3].ta3puntdesc:=11;
...
//_______________________________________________________________
procedure TR502ProDtM1.z1004_ArrotondaOreCausalizzate(Dopo_CompensoDebitoDaCausEscluse:Boolean = False);
...
begin
  ...
  try
    if triepgiuspres[i].tcauspres = '' then
      Continue;
    if (not Dopo_CompensoDebitoDaCausEscluse) and (ValStrT275[triepgiuspres[i].tcauspres,'ARROT_RIEPGG_FINECONT'] = 'S') then
      Continue;
    if (Dopo_CompensoDebitoDaCausEscluse) and (ValStrT275[triepgiuspres[i].tcauspres,'ARROT_RIEPGG_FINECONT'] = 'N') then
      Continue;
    ...
    begin
      ...
      if (ValStrT275[triepgiuspres[i].tcauspres,'ORENORMALI'] = 'A') then
        dec(minlavesc,Resto);
      //Se causale esclusa e il resto dell'arrotondamento viene perso, si devono togliere le ore da tminlav nelle fasi giuste
...
var (*i,j,Tot,OreMax,OreArr,mm,OreEscluse:Integer;*)
    (*ArrCaus:array of t_tcausale;*)
    locEcc:t_FasceInteri;
    xx:Integer;
  procedure CausalizzaEccedenza(locTipo,locCausali:String);
  var i,j,Tot,OreMin,OreMax,OreArr,mm,OreEscluse,OreIncluse:Integer;
      ArrCaus:array of t_tcausale;
  begin
    OreEscluse:=0;
    OreIncluse:=0;
    ...
    with TStringList.Create do
    try
      CommaText:=locCausali;//cdsT020.FieldByName('CAUSALI_ECCEDENZA').AsString;
    ...
    begin
      OreMin:=StrToIntDef(ValStrT275[ArrCaus[i].tcaus,'MINMINUTI'],0);
      ...
      Tot:=R180SommaArray(locEcc);//R180SommaArray(tminstrgio);
      if OreArr > 1 then
      ...
      begin
        ...
        if riepcausioe = 'A' then
          ...
          OreIncluse:=max(OreIncluse,Tot);
        z810_rieppres(OreMax);
        if i810 > 0 then
          for j:=1 to n_fasce do
          begin
            mm:=min(locEcc[j],Tot);//min(tminstrgio[j],Tot);
    ...
    if (OreEscluse > 0) then
      inc(eccliqesc,max(0,OreEscluse - OreIncluse));
    if OreEscluse > 0 then
    begin
      for i:=1 to n_fasce do
      begin
        mm:=min(locEcc[i],OreEscluse);//min(tminstrgio[i],OreEscluse);
        dec(OreEscluse,mm);
        dec(locEcc[i],mm);//dec(tminstrgio[i],mm);
        if locTipo = 'ECC' then
        begin
...
begin
  if (R180SommaArray(tminstrgio) > 0) and (cdsT020.FieldByName('CAUSALI_ECCEDENZA').AsString <> '') then
  begin
    locEcc:=tminstrgio;
    CausalizzaEccedenza('ECC',cdsT020.FieldByName('CAUSALI_ECCEDENZA').AsString);
    tminstrgio:=locEcc;
  end;
  if (eccsolocomp > 0) and (cdsT020.FieldByName('CAUSALI_ECCCOMP').AsString <> '') then
  begin
    for xx:=Low(locEcc) to High(locEcc) do locEcc[xx]:=0;
    locEcc[1]:=eccsolocomp;
    CausalizzaEccedenza('ECCCOMP',cdsT020.FieldByName('CAUSALI_ECCCOMP').AsString);
    eccsolocomp:=locEcc[1];
  end;
end;
...
procedure TR502ProDtM1.z1013_ArrotondaOreCausalizzateSGM(Dopo_CompensoDebitoDaCausEscluse:Boolean = False);
