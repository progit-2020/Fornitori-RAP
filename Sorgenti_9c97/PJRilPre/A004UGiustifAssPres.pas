...
begin
  ...
  begin
    chkNuovoPeriodo.Checked:=False;
    raise Exception.Create(A000TraduzioneStringhe(A000MSG_ERR_NO_DIP));//R180MessageBox(A000TraduzioneStringhe(A000MSG_ERR_NO_DIP),ESCLAMA);
    Exit;
  end;
  ...
  begin
    try
      ...
      if VarIsNull(LVData) then
        ...
        LDataNas:=R180VarToDateTime(LVData);
      ...
      begin
        LDataAdoz:=R180VarToDateTime(LVData);
...
begin
  C:=ECausale.KeyValue;
  A004MW.Var_Gestione:=rgpGestione.ItemIndex;
  A004MW.Q265.Filtered:=False;
  A004MW.Q265.Filtered:=True;
  ECausale.KeyValue:=C;
...
begin
  if TryStrToDate(EDaData.Text,T047DataInizio) then
    ...
    raise Exception.Create(A000MSG_ERR_DATA_INIZIO_PERIODO_VUOTA);//R180MessageBox('Data di inizio periodo non valida!',ERRORE);
end;
