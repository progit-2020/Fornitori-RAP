...
uses
  ...
  C008UCercaDuplicati, C009UCopiaSu, C013UCheckList, C180FunzioniGenerali, R500Lin,
...
type
  TA006FModelliOrario = class(TR004FGestStorico)
    ...
    lblCausaliEccComp: TLabel;
    btnCausaliEccComp: TButton;
    dedtCausaliEccComp: TDBEdit;
    dchkSeparaTimbConteggi: TDBCheckBox;
...
begin
  ...
  btnCausaliEccComp.Enabled:=DButton.State in [dsInsert,dsEdit];
...
begin
  ...
  with TC013FCheckList.Create(nil) do
    try
      ...
      else if Sender = btnCausaliEccComp then
        R180PutCheckList(A006FModelliOrarioDtM1.selT020.FieldByName('CAUSALI_ECCCOMP').AsString,5,clbListaDati);
      if ShowModal = mrOK then
        if DButton.State in [dsEdit,dsInsert] then
        begin
          if Sender = btnCausaliEccedenza then
            ...
            A006FModelliOrarioDtM1.selT020.FieldByName('CAUSALI_ECCCOMP').AsString:=R180GetCheckList(5,clbListaDati)
        end;
    finally
      Free;
    end;
