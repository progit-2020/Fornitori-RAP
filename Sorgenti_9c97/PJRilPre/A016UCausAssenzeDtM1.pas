...
uses
  ...
  C180FunzioniGenerali, Variants, ControlloVociPaghe, StrUtils, A016UCausAssenzeMW,
  A016UCausAssenzeStoricoMW, System.Generics.Collections;
...
type
  TA016FCausAssenzeDtM1 = class(TDataModule)
    ...
    T265CSI_MAX_MGMAT: TIntegerField;
    T265CSI_MAX_MGPOM: TIntegerField;
    T265CSI_MAX_MG: TIntegerField;
    T265ID: TIntegerField;
    T265ESTENDE_MALATTIA: TStringField;
...
begin
  ...
  begin
    ...
    //PFasce.isible:=(AnnoS = 'N') and (not (R180CarattereDef(T265.FieldByName('TipoCumulo').AsString,1,'H') in ['H','N','P','Q','R','S','T']));
...
begin
  ...
  //  20/10/2016 Manuele Dentello - aggiunto controllo sui limiti di fruizione mezze giornate
  if not T265.FieldByName('CSI_MAX_MG').IsNull then
  begin
    if T265.FieldByName('CSI_MAX_MG').AsInteger > T265.FieldByName('CSI_MAX_MGMAT').AsInteger + T265.FieldByName('CSI_MAX_MGPOM').AsInteger then
      raise Exception.Create('Il numero massimo di mezze giornate non può superare la somma delle mattine e dei pomeriggi');
  end;
  if (T265.FieldByName('INFLUCONT').AsString = 'D') and (T265.FieldByName('DETREPERIB').AsString <> '0') then
    T265.FieldByName('DETREPERIB').AsString:='1';
...
begin
  ...
  // ALESSANDRIA_COMUNE - commessa 2015/214 SVILUPPO#1.fine
    ...
    begin
      ...
      ImpostaPartTime;
      // ALESSANDRIA_COMUNE - commessa 2015/214 SVILUPPO#1.ini
      LFruizioneMezzaGG:=T265.FieldByName('UM_INSERIMENTO_MG').AsString = 'S';
      lblCSIMaxMG.Enabled:=LFruizioneMezzaGG;     
      dedtCSIMaxMG.Enabled:=LFruizioneMezzaGG;     
      lblCSIMaxMGMat.Enabled:=LFruizioneMezzaGG;
      dedtCSIMaxMGMat.Enabled:=LFruizioneMezzaGG;
      lblCSIMaxMGPom.Enabled:=LFruizioneMezzaGG;
      dedtCSIMaxMGPom.Enabled:=LFruizioneMezzaGG;
      // ALESSANDRIA_COMUNE - commessa 2015/214 SVILUPPO#1.fine
    end;
    RefreshCodiceVoce;
