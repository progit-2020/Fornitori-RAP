...
begin
  ...
  begin
    ...
    begin
      ...
      dchkDetReperibIntera.Enabled:=(R180CarattereDef(dcmbDetReperib.Text) in ['1','2']) and (DBComboBox1.Text <> 'D');
...
begin
  ...
  begin
    EUMCumulo.Caption:='Unità di misura';
    EUMCumulo.Columns:=2;
    EUMCumulo.Items.Clear;
...
begin
  if (DButton.State in [dsEdit,dsInsert]) and (dgrpCumuloGlobale.ItemIndex <> 3) then
    ERaggruppamento.Field.Clear;
  ERaggruppamento.Enabled:=dgrpCumuloGlobale.ItemIndex = 3;
  LRaggruppamento.Enabled:=dgrpCumuloGlobale.ItemIndex = 3;
end;
...
begin
  inherited;
  {selVSource viene aperto sull'evento onPopUp del popupmenù}
...
begin
  inherited;
  dchkDetReperibIntera.Enabled:=(dcmbDetReperib.ItemIndex in [1,2]) and (DBComboBox1.Text <> 'D');
end;
