...
type
  TAc01FProgettiRendiProj = class(TR004FGestStorico)
    ...
    dedtCUP: TDBEdit;
    Azioni1: TMenuItem;
    actCopiaDettaglio: TAction;
    Copiadettaglioda1: TMenuItem;
    ...
    procedure actCopiaDettaglioExecute(Sender: TObject);
  private
    ...
    try
      ...
      begin
        ...
        begin
          ...
          C013FCheckList.clbListaDati.Checked[C013FCheckList.clbListaDati.Count - 1]:=FieldByName('RENDICONTA_PROGETTI').AsString = 'S';
          C013FCheckList.clbListaDati.ItemEnabled[C013FCheckList.clbListaDati.Count - 1]:=False;
          Next;
        end;
      end;
      C013FCheckList.BitBtn1.Enabled:=False; //E' una semplice visualizzazione di tutte le causali di assenza e di presenza a cui è stato attivato il parametro storicizzato RENDICONTA_PROGETTI
...
begin
  ...
  begin
    ...
    begin
      ...
      actCopiaDettaglio.Enabled:=(selT750.RecordCount > 0) and (cmbDateDecorrenza.Items.Count = 1) and (selT751.RecordCount = 0) and not SolaLettura;
...
procedure TAc01FProgettiRendiProj.actCopiaDettaglioExecute(Sender: TObject);
var
  vCodice:Variant;
  IdCopia:Integer;
  ErrMsg:String;
begin
  ...
  with Ac01FProgettiRendiProjDtM.Ac01MW do
  begin
    //Faccio segliere l'ultima storicizzazione dei progetti che hanno almeno un'attività
    selT750Copia.Close;
    selT750Copia.Open;
    IdCopia:=-1;
    vCodice:=VarArrayOf(['']);
    OpenC015FElencoValori('SG700_VALUTAZIONI','<Ac01> Progetto di cui copiare il dettaglio',selT750Copia.SQL.Text,'ID',vCodice,selT750Copia,700);
    if not VarIsNull(vCodice) then
      IdCopia:=StrToIntDef(VarToStr(vCodice[0]),-1);
    //Copio tutte le attività, task e dipendenti dell'unico progetto selezionato
    //Le decorrenze interne dal/al indicate nella sezione dipendenti dovranno essere aggiornate con il valore di decorrenza (dal/al) indicato per il nuovo progetto
    if IdCopia > -1 then
    begin
      scrCopiaDettaglio.ClearVariables;
      scrCopiaDettaglio.SetVariable('ID_T750_DA',IdCopia);
      scrCopiaDettaglio.SetVariable('ID_T750_A',selT750.FieldByName('ID').AsInteger);
      scrCopiaDettaglio.Execute;
      ErrMsg:=VarToStr(scrCopiaDettaglio.GetVariable('ERR'));
      if ErrMsg <> '' then
        raise exception.Create(A000MSG_MSG_OPERAZIONE_ANNULLATA + CRLF + ErrMsg)
      else
      begin
        actRefreshExecute(nil);//Aggiorna la lista dei dipendenti e le tabelle figlie collegate
