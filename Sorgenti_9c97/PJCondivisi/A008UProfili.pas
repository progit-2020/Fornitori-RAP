...
begin
  if not R180In(cmbDizionario.Text,['ANOMALIE DEI CONTEGGI','ANOMALIE NASCOSTE SU CARTELLINO WEB','MANIPOLAZIONE DATI']) then
    ...
    begin
      ...
      while not Eof do
      begin
        lstDizionario.Items.Add(IfThen(cmbDizionario.Text = 'ITER AUTORIZZATIVI',
                                       A000GetDescIter(FieldByName('CODICE').AsString),
  ...
  for i:=0 to lstDizionario.Items.Count - 1 do
    if not R180In(cmbDizionario.Text,['ANOMALIE DEI CONTEGGI','ANOMALIE NASCOSTE SU CARTELLINO WEB']) then
      lstDizionario.Checked[i]:=A008FOperatoriDtM1.selI074.SearchRecord('TABELLA;CODICE',VarArrayOf([cmbDizionario.Text, IfThen(cmbDizionario.Text = 'ITER AUTORIZZATIVI',
                                                                                                                         A000GetCodIter(lstDizionario.Items[i]),
                                                                                                                         lstDizionario.Items[i])]),[srFromBeginning])
    else
