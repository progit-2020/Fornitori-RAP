...
type
  ...
  TA008FOperatoriDtM1 = class(TDataModule)
    ...
    procedure BeforeOpen_ReadBuffer(DataSet: TDataSet);
  private
...
begin
  ...
  begin
    ...
    Parametri.CampiRiferimento.C8_W032MaxAllegati:=VarToStr(QI091.Lookup('Tipo','C8_W032_MAX_ALLEGATI','Dato'));
    ...
    Parametri.CampiRiferimento.C37_NumColAltriProg:=VarToStr(QI091.Lookup('Tipo','C37_NUM_COL_ALTRI_PROG','Dato'));
    Parametri.CampiRiferimento.C37_SoloAttRend:=VarToStr(QI091.Lookup('Tipo','C37_SOLO_ATT_REND','Dato'));
    Parametri.CampiRiferimento.C37_TipoTempo:=VarToStr(QI091.Lookup('Tipo','C37_TIPO_TEMPO','Dato'));
    Parametri.CampiRiferimento.C37_SezGiustRitardi:=VarToStr(QI091.Lookup('Tipo','C37_SEZ_GIUST_RITARDI','Dato'));
    Parametri.CampiRiferimento.C37_CUP:=VarToStr(QI091.Lookup('Tipo','C37_CUP','Dato'));
    Parametri.CampiRiferimento.C37_Logo:=VarToStr(QI091.Lookup('Tipo','C37_LOGO','Dato'));
    ...
    Parametri.CampiRiferimento.C90_W026SpezzoniNonCaus:=VarToStr(QI091.Lookup('Tipo','C90_W026SPEZZONI_NONCAUS','Dato'));
    ...
    Parametri.CampiRiferimento.C90_CancellaAnnoAllegatiIter:=VarToStr(QI091.Lookup('Tipo','C90_CANCELLA_ANNO_ALLEGATI_ITER','Dato'));
    Parametri.CampiRiferimento.C90_PreavvisoDelAllegati:=VarToStr(QI091.Lookup('Tipo','C90_GG_PREAVVISO_CANCELLA_ALLEGATI','Dato'));
...
procedure TA008FOperatoriDtM1.BeforeOpen_ReadBuffer(DataSet: TDataSet);
begin
...
begin
  ...
  while not selI072.Eof do
    selI072.Delete;
  if selI072Dist.FieldByName('PROFILO').AsString <> '' then
  begin
    RegistraLog.SettaProprieta('C','I072_FILTROANAGRAFE','A008',nil,True);
    RegistraLog.InserisciDato('PROFILO',selI072Dist.FieldByName('PROFILO').AsString,'');
    RegistraLog.RegistraOperazione;
  end;

  if Trim(A008FProfili.memoFiltroAnagrafe.Text) <> ''  then
  ...
  begin
    ...
    selI072.Post;
    if selI072.FieldByName('FILTRO').AsString.Trim <> '' then
    begin
      RegistraLog.InserisciDato('PROFILO','',selI072.FieldByName('PROFILO').AsString);
      RegistraLog.InserisciDato('PROGRESSIVO','',selI072.FieldByName('PROGRESSIVO').AsString);
      RegistraLog.InserisciDato('FILTRO','',selI072.FieldByName('FILTRO').AsString);
    end;
  end;
  RegistraLog.RegistraOperazione;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if A008FProfili.cmbDizionario.text = 'ITER AUTORIZZATIVI' then
        selI074.FieldByName('CODICE').AsString:=A000GetCodIter(A008FProfili.lstDizionario.Items[i])
      else if not R180In(A008FProfili.cmbDizionario.Text,['ANOMALIE DEI CONTEGGI','ANOMALIE NASCOSTE SU CARTELLINO WEB']) then
