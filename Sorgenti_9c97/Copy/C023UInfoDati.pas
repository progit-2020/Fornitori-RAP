...
uses
  ...
  A023UTimbratureDtm1, IOUtils, shellAPI,C012UVisualizzaTesto,
...
type
  TC023FInfoDati = class(TForm)
    ...
    procedure dgrdDocumentiCellClick(Column: TColumn);
    procedure dgrdDocumentiEditButtonClick(Sender: TObject);
  private
...
begin
  ...
  try
    ...
    tabInfoRichiesta.TabVisible:=PDatiGiust._IdRichiesta > 0;
    tabInfoCertificatoINPS.TabVisible:=PDatiGiust._IdCertificato <> '';
    tabLog.TabVisible:=True;

    if PDatiGiust._IdRichiesta > 0 then
    begin
      CaricaInfoRichiesta(PDatiGiust._IdRichiesta);
      tabMain.ActivePage:=tabInfoRichiesta;
    end
    else if PDatiGiust._IdCertificato <> '' then
    begin
      CaricaInfoCertificatoINPS(PDatiGiust._IdCertificato);
...
begin
  ...
  try
    // imposta visibilità dei tab
    tabInfoRichiesta.TabVisible:=PDatiTimb._IdRichiesta > 0;
    tabInfoCertificatoINPS.TabVisible:=False;
    tabLog.TabVisible:=True;

    if PDatiTimb._IdRichiesta > 0 then
    begin
      CaricaInfoRichiesta(PDatiTimb._IdRichiesta);
      tabMain.ActivePage:=tabInfoRichiesta;
    end
    else
    begin
...
var
  ...
  LstEstensioni:TStringList;
  i:integer;
  blnEstensione:Boolean;
begin
  try
    IterMaxAllegati:=StrToIntDef(Parametri.CampiRiferimento.C90_IterMaxAllegati,999);
    if IterMaxAllegati < 0 then
    ...
    begin
      OpenDlg.Filter:='';
      LstEstensioni:=TStringList.Create;
      LstEstensioni.CommaText:=Parametri.CampiRiferimento.C90_IterEstensioneAllegato;
      OpenDlg.Filter:='File ' + LstEstensioni.CommaText + '|';
      for i:=0 to LstEstensioni.Count-1 do
        OpenDlg.Filter:=OpenDlg.Filter + '*.' + LstEstensioni[i] +IfThen(i<LstEstensioni.Count-1, ';');
    end
    else
    ...
    if Parametri.CampiRiferimento.C90_IterEstensioneAllegato <> '' then
    //Per sicurezza controllo che il file selezionato abbia l'estensione corretta
    begin
      blnEstensione:=False;
      for i:=0 to LstEstensioni.Count-1 do
      begin
        blnEstensione:=IfThen(Copy(LstEstensioni[i],1,1) <> '.','.') + UpperCase(LstEstensioni[i]) = UpperCase(TPath.GetExtension(OpenDlg.FileName));
        if blnEstensione then
          break;
      end;
      FreeAndNil(LstEstensioni);
      if not blnEstensione then
        raise Exception.Create('Il file selezionato non ha un''estesione valida');
    end;

    if assigned(LstEstensioni) then
      FreeAndNil(LstEstensioni);
    ...
    try
      ...
      try
        ...
        //Log.ini
        RegistraLog.SettaProprieta('I', 'T960_DOCUMENTI_INFO',C021DM.Maschera, FDataSetIter, True);
...
begin
  ...
  dgrdDocumenti.Visible:=FC018IterInfo.EsisteGestioneAllegati;
...
begin
  ...
  try
    ...
    try
      ...
      //Log.ini
      RegistraLog.SettaProprieta('C', 'T960_DOCUMENTI_INFO',C021DM.Maschera, FDataSetIter, True);
      RegistraLog.InserisciDato('ID_ALLEGATO', LIdDoc.ToString, '');
      with dgrdDocumenti.DataSource.DataSet do
      begin
        RegistraLog.InserisciDato('NOME_FILE', R180EstraiNomeFile(FieldByName('NOME_ALLEGATO').AsString), '');
        RegistraLog.InserisciDato('EXT_FILE', R180EstraiExtFile(FieldByName('NOME_ALLEGATO').AsString), '');
        if FieldByName('NOTE_ALLEGATO').AsString <> '' then
          RegistraLog.InserisciDato('NOTE', FieldByName('NOTE_ALLEGATO').AsString, '');
      end;
      RegistraLog.RegistraOperazione;
      //Log.Fine
...
begin
  ...
  if FDatiGiust._Progressivo <> 0 then
  begin
    // cerca log giustificativo
    FA093MW.selI000_T040.Close;
    FA093MW.selI000_T040.SetVariable('PROGRESSIVO',FDatiGiust._Progressivo);
    FA093MW.selI000_T040.SetVariable('DATA',FDatiGiust._Data);
  ...
  else if FDatiTimb._Progressivo <> 0 then
  begin
    FA093MW.selI000_T100.Close;
    FA093MW.selI000_T100.SetVariable('PROGRESSIVO',FDatiTimb._Progressivo);
    FA093MW.selI000_T100.SetVariable('DATA',FDatiTimb._Data);
...
procedure TC023FInfoDati.dgrdDocumentiCellClick(Column: TColumn);
begin
  if dgrdDocumenti.DataSource.DataSet.FieldByName('ALLEGATO_BACKOFFICE').AsString = 'S' then
    dgrdDocumenti.Columns[2].ButtonStyle:=cbsEllipsis
  else
    dgrdDocumenti.Columns[2].ButtonStyle:=cbsAuto;
end;

procedure TC023FInfoDati.dgrdDocumentiEditButtonClick(Sender: TObject);
var
  Str:TStringList;
  LId: Integer;
begin
  if (dgrdDocumenti.DataSource.DataSet.FieldByName('ALLEGATO_BACKOFFICE').AsString = 'S') and
     (dgrdDocumenti.SelectedField.FieldName = 'NOTE_ALLEGATO') then
  begin
    LId:=dgrdDocumenti.DataSource.DataSet.FieldByName('ID_ALLEGATO').AsInteger;
    if LId <= 0 then
      Exit;
    ...
    try
      Str.Text:=dgrdDocumenti.DataSource.DataSet.FieldByName('NOTE_ALLEGATO').AsString;
      OpenC012VisualizzaTesto(dgrdDocumenti.SelectedField.DisplayLabel,'',Str,'',[mbOK,mbCancel]);
      if Trim(Str.Text) <> Trim(dgrdDocumenti.DataSource.DataSet.FieldByName('NOTE_ALLEGATO').AsString) then
      begin
        C018.updT960Note.SetVariable('ID',LId);
        C018.updT960Note.SetVariable('NOTE',Trim(Str.Text));
        C018.updT960Note.Execute;

        //Log.ini
        RegistraLog.SettaProprieta('M', 'T960_DOCUMENTI_INFO', C018.Chiamante, FDataSetIter, True);
        RegistraLog.InserisciDato('ID_ALLEGATO', LId.ToString, '');
        RegistraLog.InserisciDato('NOME_FILE', R180EstraiNomeFile(dgrdDocumenti.DataSource.DataSet.FieldByName('NOME_ALLEGATO').AsString), '');
        RegistraLog.InserisciDato('EXT_FILE', R180EstraiExtFile(dgrdDocumenti.DataSource.DataSet.FieldByName('NOME_ALLEGATO').AsString), '');
        RegistraLog.InserisciDato('NOTE', Trim(dgrdDocumenti.DataSource.DataSet.FieldByName('NOTE_ALLEGATO').AsString), Trim(Str.Text));
        RegistraLog.RegistraOperazione;
        SessioneOracle.Commit;
        //Log.fine

        dgrdDocumenti.DataSource.DataSet.Refresh;
      end;
