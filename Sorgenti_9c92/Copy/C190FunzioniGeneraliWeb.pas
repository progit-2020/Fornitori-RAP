...
procedure C190VisualizzaElemento(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean);
...
procedure RemoveJQAction(JQuery:TIWJQuerywidget;s:String); inline;
var
  idx: Integer;
begin
  idx:=JQuery.OnReady.IndexOf(s);
  while idx <> -1 do
  begin
    JQuery.OnReady.Delete(idx);
    idx:=JQuery.OnReady.IndexOf(s);
  end;
end;

function JQActionVisualizzaID(Nome:String;Visualizza:Boolean):String; inline;
begin
  if Visualizza then
    Result:='$(''#' + Nome + ''').show(); '
  else
...
procedure C190VisualizzaElemento(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean); inline;
var
  s: String;
begin
  //3/11/2015 Accodando continuamente a JQuery.OnReady si ottiene una stringa lunga e pesante per il browser.
  //prima di fare show o hide, rimuovo eventuali già fatte prima su stesso id
  s:=JQActionVisualizzaID(Nome,True);
  RemoveJQAction(JQuery,s);
  s:=JQActionVisualizzaID(Nome,False);
  RemoveJQAction(JQuery,s);

  JQuery.OnReady.Add(JQActionVisualizzaID(Nome,Visualizza));
end;
