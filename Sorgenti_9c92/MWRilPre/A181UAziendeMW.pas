...
begin
  ModuloIterAutorizzativi:=
    //Sempre abilitato per l'iter delle missioni
    (selI093.FieldByName('ITER').AsString = ITER_MISSIONI) or
    A000ModuloAbilitato(SessioneOracle,'ITER_AUTORIZZATIVI',QI090.FieldByName('AZIENDA').AsString);
...
begin
  selI095.FieldByName('COD_ITER').ReadOnly:=not ModuloIterAutorizzativi;
  selI095.FieldByName('DESCRIZIONE').ReadOnly:=not ModuloIterAutorizzativi;
  selI095.FieldByName('FILTRO_RICHIESTA').Visible:=ModuloIterAutorizzativi;
  selI095.FieldByName('MAX_LIV_AUTORIZZ_AUTOMATICA').Visible:=ModuloIterAutorizzativi;
end;
...
begin
  if not ModuloIterAutorizzativi then
    Abort;
