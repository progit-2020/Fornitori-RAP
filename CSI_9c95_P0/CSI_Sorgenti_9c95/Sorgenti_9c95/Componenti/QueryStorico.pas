...
uses
  ...
  Db, OracleData, Oracle, Variants, StrUtils, A000UCostanti, C180FunzioniGenerali;
...
begin
  ...
  if VarIsNull(GetVariable('DataInizio')) or (Dal <> R180VarToDateTime(GetVariable('DataInizio'))) then
  begin
    SetVariable('DataInizio',Dal);
    Close;
  end;
  if VarIsNull(GetVariable('DataFine')) or (Al <> R180VarToDateTime(GetVariable('DataFine'))) then
  begin
